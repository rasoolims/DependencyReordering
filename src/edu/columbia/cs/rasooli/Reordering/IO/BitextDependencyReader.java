...
public class BitextDependencyReader {
    ...
    public static void createTrainData(String parsedFilePath, String alignIntersectionPath, HashMap<String, String> universalMap, IndexMaps maps, HashMap<String, int[]>[] mostCommonPermutations, String path) throws Exception {
        System.err.print("Constructing frequency maps...");
            ...
            if (bitextDependency.getTrainableHeads().size() > 0) {
                for (int head : bitextDependency.getTrainableHeads()) {
                    ...
                    if (mostCommonPermutations[ind].containsKey(goldLabel)) {
                        writer.writeInt(ind);
                        writer.writeObject(goldLabel);
                        
                        for (int f = 0; f < features.length; f++) {
                            for (Object feat : features[f]) {
                                writer.writeInt(f);
                                writer.writeObject(feat);
                            }
                        }
                        writer.writeInt(-1);
                    }
