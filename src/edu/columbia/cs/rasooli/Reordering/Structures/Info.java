...
public class Info {
    HashMap<String, Float> finalWeights;
    ...
    public Info(AveragedPerceptron perceptron,HashMap<String,Integer> posOrderFrequencyDic, HashMap<String,String> universalPosMap, int topK){
        ...
        finalWeights=new  HashMap<String, Float>(perceptron.getAvgWeights().size());

        for(String feat:perceptron.getWeights().keySet()){
            float newValue=  perceptron.getWeights().get(feat)-(perceptron.getAvgWeights().get(feat)/perceptron.getIteration());
            if(newValue!=0.0)
    ...
    public Info(String modelPath)throws Exception {
        ...
        finalWeights = (HashMap<String, Float>) reader.readObject();
    ...
    public HashMap<String, Float> getFinalWeights() {
        return finalWeights;
    }
